...
/// Maximum decompressed attestation report size (10 MiB).
...
pub(super) fn decode_report(body: &str) -> Result<Vec<u8>> {
    ...
    if report_bytes.len() > MAX_DECOMPRESSED_SIZE {
        return Err(Error::AttestationVerification(format!(
            "Decompressed attestation exceeds maximum size ({} bytes)",
...
pub(super) fn validate_report_structure(report: &[u8]) -> Result<()> {
    ...
    // Report version: 2 = spec rev 1.55, 3 = rev 1.56, 5 = rev 1.58.
...
pub(super) fn validate_mbz_bytes(
    report: &[u8],
    start: usize,
    end: usize,
    field_name: &str,
...
pub(super) fn validate_signer_info(report: &[u8]) -> Result<bool> {
    let signer_info = u32::from_le_bytes(
        report[SIGNER_INFO_OFFSET..SIGNER_INFO_OFFSET + 4]
            .try_into()
            .unwrap(),
    );
...
mod tests {
    ...
    fn test_validate_report_structure_invalid_version_0() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Unsupported report version"));
    }
    ...
    fn test_validate_signer_info_non_vcek_signing_key() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("Only VCEK-signed reports"));
    }
    ...
    fn test_validate_signer_info_reserved_bits_set() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("bits 31-5 must be zero"));
    }
    ...
    fn test_validate_mbz_fields_non_zero_reserved_after_signer_info() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("reserved_after_signer_info"));
    }
    ...
    fn test_validate_mbz_fields_non_zero_reserved_before_signature() {
        ...
        assert!(result
            .unwrap_err()
            .to_string()
            .contains("reserved_before_signature"));
    }
    ...
    fn test_validate_mbz_fields_tcb_reserved_bits_set() {
        ...
        report[CURRENT_TCB_OFFSET..CURRENT_TCB_OFFSET + 8]
            .copy_from_slice(&tcb_with_reserved_bits.to_le_bytes());
        let result = validate_mbz_fields(&report);
        assert!(result.is_err());
